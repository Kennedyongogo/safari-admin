...
const InterestCreate = () => {
  ...
  const handleCreate = async () => {
    try {
      ...
      formData.append("interest_gallery_media", file);
      ...
      const response = await fetch("/api/interest-gallery/upload", {
        method: "POST",
        headers: {
      ...
      if (result.success) {
        ...
        await Swal.fire({
          title: "Success!",
          text: "Interest gallery item created successfully!",
          icon: "success",
          confirmButtonColor: "#6B4E3D",
        });
        navigate("/interest");
      } else {
        throw new Error(result.message || "Failed to create interest gallery item");
      }
    } catch (err) {
      console.error("Error creating interest gallery item:", err);
      setError(err.message || "Failed to create interest gallery item");
      await Swal.fire({
        title: "Error!",
        text: err.message || "Failed to create interest gallery item",
        icon: "error",
        confirmButtonColor: "#6B4E3D",
      });
    } finally {
            ...
            <Stack spacing={3}>
              ...
              <Box>
                ...
                <Typography variant="body2" sx={{ mb: 2, color: "text.secondary" }}>
                  Upload one image/video to represent this "By Interest" card/gallery item.
                </Typography>
                <Box mb={2}>
                  <input
                    type="file"
                    ...
                    style={{ display: "none" }}
                    id="interest-media-upload"
                  />
                  <label htmlFor="interest-media-upload">
                    <Button
                      variant="outlined"
                      component="span"
