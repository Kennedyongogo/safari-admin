...
const Form = () => {
  ...
  const fetchForms = async () => {
    try {
      ...
      if (data.success) {
        setForms(data.data || []);
        setTotalForms(data.pagination?.total || data.data?.length || 0);
      } else {
  ...
  const handleDelete = async (form) => {
    const result = await Swal.fire({
      title: "Delete form?",
      text: `"${form.title}" will be removed.`,
      icon: "warning",
    ...
    try {
      ...
      await Swal.fire({
        icon: "success",
        title: "Deleted",
        text: `"${form.title}" removed successfully.`,
        timer: 1400,
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: err.message || "Failed to delete form",
      });
    } finally {
      setLoading(false);
    }
