  ...
  const fetchItem = async () => {
    try {
      ...
      if (!res.ok || !data.success)
        throw new Error(data.message || "Failed to load traveller gallery item");
    ...
    return (
      <Container maxWidth="md" sx={{ mt: 4 }}>
        <Alert severity="error" sx={{ mb: 2 }}>
          {error || "Traveller gallery item not found"}
        </Alert>
        <Button
          variant="outlined"
          startIcon={<ArrowBack />}
          ...
          >
            <CardContent>
              ...
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, mb: 2 }}>
                <Chip label={`Category: ${item.category || "—"}`} size="small" />
                <Chip label={`Type: ${item.type || "—"}`} size="small" />
                <Chip label={`Status: ${item.isActive ? "Active" : "Inactive"}`} size="small" />
                <Chip label={`Featured: ${item.isFeatured ? "Yes" : "No"}`} size="small" />
                <Chip label={`Priority: ${item.priority ?? 0}`} size="small" />
              </Box>
