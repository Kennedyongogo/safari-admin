...
const TravellerEdit = () => {
  ...
  const fetchTravellerItem = async () => {
    try {
      setLoading(true);
      setError(null);
      const token = localStorage.getItem("token");
      const res = await fetch(`/api/traveller-gallery/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.message || "Failed to load traveller gallery item");
      ...
      setError(err.message || "Failed to load traveller gallery item");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mediaFile) {
        formData.append("traveller_gallery_media", mediaFile);
      }

      const response = await fetch(`/api/traveller-gallery/${id}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      ...
      if (data.success) {
        Swal.fire({ icon: "success", title: "Success!", text: "Traveller gallery item updated successfully", timer: 1500, showConfirmButton: false });
        navigate(`/traveller/${id}`);
      } else {
        throw new Error(data.message || "Failed to update traveller gallery item");
      }
    } catch (err) {
      console.error("Error updating traveller gallery item:", err);
      Swal.fire({ icon: "error", title: "Error", text: err.message || "Failed to update traveller gallery item" });
    } finally {
      setSaving(false);
    }
  };
          ...
          <Box display="flex" alignItems="center" gap={2} position="relative" zIndex={1}>
            ...
            <Box>
              <Typography variant="h4" sx={{ fontWeight: 800, textShadow: "0 2px 4px rgba(0,0,0,0.3)" }}>
                Edit By Traveller Item
              </Typography>
              <Typography variant="body1" sx={{ opacity: 0.9 }}>
                {form.title || "Traveller gallery item details"}
              </Typography>
            </Box>
          </Box>
        </Box>
            ...
            <form onSubmit={handleSubmit}>
              <Stack spacing={3}>
                ...
                <Box display="flex" gap={2} justifyContent="flex-end" sx={{ mt: 3 }}>
                  <Button variant="outlined" onClick={() => navigate(`/traveller/${id}`)} disabled={saving}>Cancel</Button>
